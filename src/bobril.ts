...
b = ((window: Window, document: Document): IBobrilStatic => {
    ...
    var setValueCallback: (el: Element, node: IBobrilCacheNode, newValue: any, oldValue: any) => void = (
        ...
        if (newValue !== oldValue) (<any>el)["value"] = newValue;
    };
            ...
            if (mi === undefined) {
                if (DEBUG) {
                    ...
                    if (/-/.test(ki) && window.console && console.warn)
                        console.warn(
                            "Style property " + ki + " contains dash (must use JS props instead of css names)"
                        );
                }
    ...
    function getCurrentCtxWithEvents(): IBobrilCtx | undefined {
        return currentCtxWithEvents;
    }

    function setCurrentCtxWithEvents(ctx: IBobrilCtx | undefined): void {
        currentCtxWithEvents = ctx;
    }

    function createNode(
        n: IBobrilNode,
        parentNode: IBobrilCacheNode,
        createInto: Element,
        createBefore: Node
    ): IBobrilCacheNode {
    ...
    function updateNode(
        ...
        else parEl = (<Node>parEl).parentNode;
    ...
    function reorderInUpdateChildrenRec(c: IBobrilCacheNode, element: Element, before: Node): void {
        ...
        if (el != null) {
            ...
            } else element.insertBefore(<Node>el, before);
            return;
        }
        var ch = c.children;
        if (!isArray(ch)) return null;
    ...
    function selectedUpdate(cache: IBobrilCacheNode[], element: Element, createBefore: Node) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (ctx != null && (<any>ctx)[ctxInvalidated] === frameCounter) {
                ...
                selectedUpdate(
                    <IBobrilCacheNode[]>node.children,
    ...
    function update(time: number) {
        ...
        for (var i = 0; i < rootIds.length; i++) {
            ...
            if (fullRefresh) {
                var newChildren = r.f();
                if (newChildren === undefined) break;
    ...
    function runMethod(methodId: string, params: any): boolean {
        ...
        let ctx = getCurrentCtxWithEvents();
        if (ctx)
        ...
        function runMethodFrom(currentRoot: IBobrilCacheNode | IBobrilCacheNode[], methodId: string, params: any): void {
            ...
            else
            {
                while (currentRoot) {
                    ...
                    const children: IBobrilCacheChildren = currentRoot.children;
    
                    if (isArray(children) && !done) loopChildNodes(children);
                    if (!done) {
                        const logicalChildren: IBobrilCacheNode[] = getLogicalChildren(currentRoot);
            ...
            function loopChildNodes(children: IBobrilCacheNode[]): void {
                for (let i = 0; i < children.length; i++) {
                    const child: IBobrilCacheNode = children[i];
                    if (child == previousRoot)
                        continue;
                    const comp: any = child.component;
                    if (comp && comp.runMethod) {
                        const prevCtx: IBobrilCtx = currentCtxWithEvents;
                    ...
                    isArray(child.children) && !done && loopChildNodes(child.children);
                    if (!done) {
                        const logicalChildren: IBobrilCacheNode[] = getLogicalChildren(child);
                        isArray(logicalChildren) && loopChildNodes(logicalChildren);
                    }
                    if (done)
                        return;
        ...
        function getLogicalChildren(elm: any): IBobrilCacheNode[] {
    ...
    return {
        ...
        setStyleShim: (name: string, action: (style: any, value: any, oldName: string) => void) =>
            (mapping[name] = action),
        init: init,
        addRoot: addRoot,
